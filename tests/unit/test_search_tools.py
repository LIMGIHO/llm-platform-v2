...
from app.mer_persona.schemas.search import MarketSearchRequest, ToolName, WebSearchRequest
...
async def test_disabled_web_tool_fails_clearly():
    ...
    assert tool.name is ToolName.WEB
...
async def test_disabled_market_tool_fails_clearly():
    ...
    assert tool.name is ToolName.MARKET
    with pytest.raises(ToolExecutionError) as exc:
